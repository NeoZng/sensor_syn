...
int main(int argc, char **argv) {
    ...
    int fd = open(path_for_time_stamp.c_str(), O_RDWR, 0666);
    if (fd == -1) {
        ROS_ERROR("[camera] failed to open share mm");
    }
    ...
    while (ros::ok()) {
        ...
        while (ros::ok()) {
            ...
            if (nRet == MV_OK) {
                ...
                pub.publish(msg);
                ROS_INFO("publish one frame:%d",after_PPS_cnt);
                // ROS_INFO("publish one frame");
